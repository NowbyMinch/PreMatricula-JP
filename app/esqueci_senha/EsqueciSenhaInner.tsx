...
export default function Home() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const step3 = {
      password: formStep3.password,
      confirmPassword: formStep3.confirmPassword,
    };
  ...
  const Reenviar = async (e: React.FormEvent) => {
    ...
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/home/password/reset/resend`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(step2),
    ...
    <>
      ...
      >
        ...
        <form
          ...
          className={`w-full flex flex-col items-center text-white max-w-[90%] h-full my-10 transition-all ease-in-out duration-300`}
        >
          <motion.img
            initial={{ scale: 0 }}
            animate={{ scale: 1 }}
          ...
          <motion.h1
            ...
            animate={{ scale: 1 }}
            exit={{ scale: 0 }}
            className="text-[40px] mx-auto my-5 font-medium"
          >
            Esqueci a senha
          </motion.h1>
          ...
          >
            {step === 1
              ? "Insira a conta para a qual será enviado o email de redefinição de senha."
              : step === 2
              ? "Insira novamente o email e o código enviado para caixa de entrada."
              : step === 3
              ? "Insira a nova senha."
              : ""}
          </motion.p>
            ...
            <>
                    ...
                    <div className="flex gap-2">
                      {Array.from({ length: 5 }).map((_, i) => (
                        <motion.input
                          ...
                          inputMode="numeric"
                          placeholder=""
                          className={` w-[40px] text-[20px] max-w-full rounded-[10px] px-3 py-3 border outline-none transition-all ease-in-out duration-300 border-gray-400 focus:border-yellow-400 focus:shadow-[0_0_15px_rgba(255,215,0,0.2)] `}
                        />
                      ))}
                    </div>
                  </motion.div>
              ...
              {step === 3 && (
                <>
                  <motion.label
                    htmlFor=""
                    className="origin-left mt-4 w-[480px]"
                  >
                    Senha
                  </motion.label>
                  ...
                  <motion.label
                    htmlFor=""
                    className="origin-left mt-5 w-[480px]"
                  >
                    Confirmar senha
                  </motion.label>
                  <div className="relative w-[480px] max-w-full flex flex-col gap-5 mx-auto mt-2">
