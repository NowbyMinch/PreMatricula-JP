...
export default function Home() {
    ...
    return (
        ...
        <div className={` ${registrar ? "h-[550px]" :"h-[651px]"} overflow-hidden max-h-[95%] max-w-[95%] w-[600px] transition-all ease-in-out duration-300 rounded-[25px] flex justify-center items-center bg-[rgba(12,12,14,0.985)] gap-4 z-20 flex-col shadow-2xl`}>
            <form className={`w-full flex flex-col items-center text-white max-w-[90%] h-full `} onSubmit={handleSubmit}>
                ...
                animate={{scale:1}}
                exit={{scale:0}}
                className="text-[50px] mx-auto my-5 font-medium">{registrar ? "Registrar" : "Login" }</motion.h1>
                ...
                <div className="flex flex-col w-[480px] max-w-full h-full  items-center">
                    ...
                    type="submit"
                    className="cursor-pointer rounded-[15px] my-auto w-fit max-w-full px-14 py-2 bg-gradient-to-r from-yellow-500 to-yellow-400 text-lg text-black font-semibold ">{registrar ? "Registrar":"Entrar"}</motion.button>
                    
                    <motion.span 
                    initial={{scale:0}}
                    animate={{scale:1}}
                    exit={{scale:0}}
                    onClick={() => { if (registrar) {setRegistrar(false)} else (setRegistrar(true))}} className="mx-auto pb-2">{registrar ? "Já possui uma conta? " : "Não possui uma conta?" } <motion.button 
                        whileHover={{scale:1.01}}
                        whileTap={{scale:0.99}}
