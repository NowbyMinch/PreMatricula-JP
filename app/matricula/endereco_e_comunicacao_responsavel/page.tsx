...
export default function Home() {
    ...
    try {
      const fetchToken = async () => {
        ...
        if (!preset.completo && PresetValor > AtualValor) {
          ...
          setRua(preset.segundoResponsavel.endereco.rua);
          setNumero(preset.segundoResponsavel.endereco.numero);
          setCidade(preset.segundoResponsavel.endereco.cidade);
          setBairro(preset.segundoResponsavel.endereco.bairro);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (dataRes?.error) {
      ...
      if (
        dataRes.message ===
        "Etapa 2B (endereço do segundo responsável) concluída com sucesso."
      ) {
                ...
                <div className="flex flex-col justify-between w-full gap-5">
                      ...
                      <motion.input
                        ...
                        className={`${
                          disable
                            ? "bg-gray-700 opacity-[0.15] text-gray-400 cursor-not-allowed "
                            : "focus:border-yellow-400 focus:shadow-[0_0_15px_rgba(255,215,0,0.2)]"
                        }  w-full rounded-[15px] px-4 py-3 border outline-none transition-all ease-in-out duration-300 border-gray-400 max-w-[480px] focus:border-yellow-400focus:shadow-[0_0_15px_rgba(255,215,0,0.2)]`}
                      />
                    </motion.div>
                  </div>
                  ...
                  >
                    ...
                    <span className="ml-2 select-none">
                      O segundo responsável mora junto com o responsável
                      financeiro
                    </span>
                  </motion.label>
                </div>
